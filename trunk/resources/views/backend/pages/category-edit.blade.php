...
<h1>编辑{{ Category::$typeTitle[Input::get('type', 1)] }}<a href="{{ url('admin/category/all') }}?type={{ Input::get('type', 1) }}" class="btn btn-info">返回列表</a>
</h1>
@stop
@section('content')
<div class="row">
  <form action="{{ url('admin/category/update') }}?type={{ Input::get('type', 1) }}" method="post" accept-charset="utf-8" class="form-horizontal">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-6">
        <input type="text" name="name" class="form-control" placeholder="名称必须唯一" value="{{ Input::old('name', $categoryInfo->name) }}"></div>
    </div>
    <br>
    <div class="clearfix">
    ...
    <div class="clearfix">
      ...
      <div class="col-sm-5">
        <div class="input-group">
          <input type="url" id="img-url" data-preview="#photo-view-img" placeholder="填写图片URL或者选择本地文件上传" name="icon" value="{{ Input::old('icon', $categoryInfo->icon) }}" class="url-image form-control">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-2">
        <input type="text" name="sort" class="unsigned-int-input form-control" maxlength='5' placeholder="0~10000" value="{{ Input::old('sort', $categoryInfo->sort) }}" >
      </div>
