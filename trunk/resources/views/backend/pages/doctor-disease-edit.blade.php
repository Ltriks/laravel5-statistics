...
<?php $doctor_id = Request::Input('doctor_id') ?>
<?php $hospital_id = Request::Input('hospital_id') ?>
<h1>医生->病症管理<span class="middle-sep mg-x-10"></span><a href="{{ url('admin/doctor/all') }}?doctor_id={{ $doctor_id }}&hospital_id={{ $hospital_id }}" class="btn btn-info">返回</a>
</h1>
@stop
@section('content')
<div class="row">
  <form action="{{ url('admin/doctor-disease/update') }}" method="post" accept-charset="utf-8" class="form-horizontal">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-6">
        <input type="text" name="name" class="form-control" placeholder="姓名" value="{{ Input::old('name', $doctorInfo->name) }}" readonly="true">
      </div>
    </div>
    <?php use App\Models\Category; ?>
    <?php use App\Models\DoctorDisease; ?>
    <?php $doctorDiseaseList = DoctorDisease::getDoctorDiseaseList($doctor_id);?>
      ...
      <div class="col-sm-6 panel panel-default" style="padding-top:15px;margin-left: 1em;margin-top: 1em;">
          ...
          <div class="panel panel-default">
            ...
            <div class="col-sm-12" style="clear:both;">
              <?php $disease_symptoms = Input::old('disease_symptoms', $doctorDiseaseList) ?>
              @foreach($v['list'] as $k2 => $v2)
              <label style="display:inline-block"><span><input type="checkbox" style="margin-right:5px;" name="disease_symptoms[]" value="{{ $v2['id'] }}" @if(in_array($v2['id'], $doctorDiseaseList)) checked @endif />{{ $v2['name'] }}</span></label>
              @endforeach
            </div>
          </div>
          @endforeach
