...
<h1>新建{{ Tag::$nameList[$type] }}标签<span class="middle-sep mg-x-10"></span><a href="{{ url('admin/tag/all') }}?type={{ $type }}" class="btn btn-info">返回列表</a>
</h1>
@stop
@section('content')
<div class="row">
  <form action="{{ url('admin/tag/create') }}" method="post" accept-charset="utf-8" class="form-horizontal">
    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
    <div class="form-group">
      <label class="col-sm-2 control-label"><i class="required-star">*</i>名称</label>
      <div class="col-sm-6">
        <input type="text" name="name" class="form-control" placeholder="名称必须唯一并且长度为4" value="{{ Input::old('name') }}" maxlength="4"></div>
    </div>
    <!-- <div class="form-group">
