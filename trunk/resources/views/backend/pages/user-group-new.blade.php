...
@section('page_title')
<h1>添加用户组<span class="middle-sep mg-x-10"></span><a href="{{ url('admin/user-group/all') }}" class="btn btn-info">返回列表</a>
</h1>
@stop
@section('content')
<div class="row">
    <form action="{{url('/admin/user-group/create')}}" method="post" accept-charset="utf-8" class="form-horizontal">
            ...
            <div class="col-sm-8">
                ...
                <div class="permissions">
                    ...
                    <div class="checkbox permissions-inline">
                        <label><input type="checkbox" class="chkbox" name="permission[]" value="{{$engName}}.{{$actionKey}}" @if(in_array("$engName.$actionKey", Input::old('permission', array()))) checked @endif>{{$action['label']}}</label>
                    </div>
                    @endforeach
                    <div class="desc gray">
