...
@section('page_title')
<h1>编辑医院<span class="middle-sep mg-x-10"></span><a href="{{ url('admin/hospital/all') }}" class="btn btn-info">返回列表</a>
</h1>
@stop
@section('content')
<?php use App\Models\Hospital; ?>
<div class="row">
  <form action="{{ url('admin/hospital/update') }}" method="post" accept-charset="utf-8" class="form-horizontal">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-6">
        <input type="text" name="name" class="form-control" placeholder="名称必须唯一" value="{{ Input::old('name', $hospitalInfo->name) }}"></div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-3">
        <select class="selectboxit" name="level">
          ...
          <option value="{{ $k }}" @ @if($k == Input::old('level', $hospitalInfo->level)) selected @endif>{{ $v }}</option>
          @endforeach
        </select>
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-3">
        <select class="selectboxit" name="type">
          ...
          <option value="{{ $k }}" @if($k == Input::old('type', $hospitalInfo->type)) selected @endif>{{ $v }}</option>
          @endforeach
        </select>
      </div>
    </div>
    ...
    <div class="clearfix">
      ...
      <div class="col-sm-5">
        <div class="input-group">
          <input type="url" id="img-url" data-preview="#photo-view-img" placeholder="填写图片URL或者选择本地文件上传" name="image" value="{{ Input::old('image', $hospitalInfo->image) }}" class="url-image form-control">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-6">
            <textarea name="intro" class="form-control" placeholder="" style="height: 200px;">{{ Input::old('intro', $hospitalInfo->intro) }}</textarea>
        </div>
    </div>
    <div class="form-group">
      <label class="col-sm-2 control-label">地址</label>
      <div class="col-sm-6">
        <input type="text" name="address" class="unsigned-int-input form-control" max='30' placeholder="医院地址" value="{{ Input::old('address', $hospitalInfo->address) }}" >
      </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-2">
        <input type="text" name="sort" class="unsigned-int-input form-control" max='30' placeholder="0~10000" value="{{ Input::old('sort', $hospitalInfo->sort) }}" >
      </div>
      <div class="col-md-4" style=" padding-top: 5px; "></div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-2">
        <input type="text" name="price" class="unsigned-int-input form-control" max='30' placeholder="价格" value="{{ Input::old('price', $hospitalInfo->price) }}" >
      </div>
      <div class="col-md-4" style=" padding-top: 5px; "></div>
    </div>
    <hr>
