...
<h1>添加科室<span class="middle-sep mg-x-10"></span><a href="{{ url('admin/hospital-department/all') }}?hospital_id={{ $hospital_id }}" class="btn btn-info">返回列表</a>
</h1>
@stop
@section('content')
<div class="row">
  <form action="{{ url('admin/hospital-department/create') }}" method="post" accept-charset="utf-8" class="form-horizontal">
      ...
      <div class="col-sm-6 panel panel-default" style="padding-top:15px;margin-left: 1em;margin-top: 1em;">
          <div class="panel panel-default">
            ...
            <div class="col-sm-12" style="clear:both;">
              ...
              <label style="display:inline-block"><span><input type="radio" style="margin-right:5px;" name="hospital_id" value="{{ $v->id }}" @if(Input::old('hospital_id', $hospital_id) == $v->id) checked @endif />{{ $v->name }}</span></label>
              @endforeach
            </div>
          </div>
      </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-3">
        <select class="selectboxit" name="parent_id">
          ...
          <option value="{{ $v->id }}" @if($v->id == Input::old('parent_id')) selected @endif>{{ $v->name }}</option>
          @endforeach
        </select>
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-6">
        <input type="text" name="name" class="form-control" placeholder="科室名称唯一" value="{{ Input::old('name') }}">
      </div>
    </div>
    <br>
    <div class="clearfix">
    ...
    <div class="clearfix">
      ...
      <div class="col-sm-5">
        <div class="input-group">
          <input type="url" id="img-url" data-preview="#photo-view-img" placeholder="填写图片URL或者选择本地文件上传" name="photo" value="{{ Input::old('photo') }}" class="url-image form-control">
