...
<h1>添加医生<span class="middle-sep mg-x-10"></span><a href="{{ Cache::get('referer') }}" class="btn btn-info">返回列表</a>
</h1>
@stop
@section('content')
<div class="row">
  <form action="{{ url('admin/doctor/create') }}" method="post" accept-charset="utf-8" class="form-horizontal">
    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
    <div class="form-group">
      <label class="col-sm-2 control-label"><i class="required-star">*</i>姓名</label>
      <div class="col-sm-6">
        <input type="text" name="name" class="form-control" placeholder="姓名" value="{{ Input::old('name') }}">
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-3">
        <select class="selectboxit" name="gender">
          ...
          <option value="{{ $k }}" @if($k == Input::old('gender')) selected @endif>{{ $v }}</option>
          @endforeach
        </select>
      </div>
    </div>
    ...
    <div class="clearfix">
      ...
      <div class="col-sm-5">
        <div class="input-group">
          <input type="url" id="img-url" data-preview="#photo-view-img" placeholder="填写图片URL或者选择本地文件上传" name="avatar" value="{{ Input::old('avatar') }}" class="url-image form-control">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            <select name="hospital_id" class="selectboxit">
              ...
              <option @if(Input::old('hospital_id', $hospital_id) == $v->id) selected @endif value="{{ $v->id }}">{{ $v->name }}</option>
              @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-6">
            <textarea name="brief" class="form-control" placeholder="" style="height: 200px;">{{ Input::old('brief') }}</textarea>
        </div>
    </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-6">
        <input type="text" name="title" class="form-control" placeholder="职称" value="{{ Input::old('title') }}">
      </div>
    </div>
    <div class="form-group">
      <label class="col-sm-2 control-label"><i class="required-star">*</i>工作年限</label>
      <div class="col-sm-3">
        <input type="text" name="experience" class="form-control" placeholder="工作年限" value="{{ Input::old('experience') }}">
      </div>
    </div>
    <div class="form-group">
      <label class="col-sm-2 control-label"><i class="required-star">*</i>毕业学校</label>
      <div class="col-sm-3">
        <input type="text" name="school" class="form-control" placeholder="毕业学校" value="{{ Input::old('school') }}">
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-3">
        <select class="selectboxit" name="is_verified">
          ...
          <option value="{{ $k }}" @if($k == Input::old('is_verified')) selected @endif>{{ $v }}</option>
          @endforeach
        </select>
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-3">
        <input type="text" name="price" class="form-control" placeholder="挂号费" value="{{ Input::old('price') }}">
      </div>
    </div>
    <hr />
    ...
    <div class="form-group">
      ...
      <div style="margin-left: 15em;margin-top: 1em;" class="col-sm-10">
        <?php $DoctorScheduleList = DoctorSchedule::getList();?>
